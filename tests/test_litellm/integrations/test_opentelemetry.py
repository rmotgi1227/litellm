...
class TestOpenTelemetrySuccessSpanDedupe(unittest.TestCase):
    """streaming code paths trigger both sync and async success
    callbacks for one request, causing duplicate litellm_request spans.
    _mark_success_span_once provides a request-local idempotency guard."""
    ...
    def test_mark_success_span_once_first_call_returns_true(self):
        otel = OpenTelemetry()
        kwargs = self._build_kwargs()
        self.assertTrue(otel._mark_success_span_once(kwargs))

    def test_mark_success_span_once_second_call_returns_false(self):
        otel = OpenTelemetry()
        kwargs = self._build_kwargs()
        self.assertTrue(otel._mark_success_span_once(kwargs))
        self.assertFalse(
            otel._mark_success_span_once(kwargs),
            "Repeat call for same handler+kwargs must be deduped",
        )

    def test_mark_success_span_once_separate_handlers_each_emit(self):
        """Two distinct handler instances must each emit exactly once."""
        otel_a = OpenTelemetry()
        otel_b = OpenTelemetry()
        kwargs = self._build_kwargs()
        self.assertTrue(otel_a._mark_success_span_once(kwargs))
        self.assertTrue(
            otel_b._mark_success_span_once(kwargs),
            "Different handler instance must not share the first handler's marker",
        )
        self.assertFalse(otel_a._mark_success_span_once(kwargs))
        self.assertFalse(otel_b._mark_success_span_once(kwargs))

    def test_mark_success_span_once_handles_missing_metadata(self):
        otel = OpenTelemetry()
        kwargs = {"litellm_params": {}}
        self.assertTrue(otel._mark_success_span_once(kwargs))
        self.assertFalse(otel._mark_success_span_once(kwargs))

    def test_mark_success_span_once_handles_missing_litellm_params(self):
        otel = OpenTelemetry()
        kwargs = {}
        self.assertTrue(otel._mark_success_span_once(kwargs))
        self.assertFalse(otel._mark_success_span_once(kwargs))
    ...
    def test_handle_success_emits_single_litellm_request_span_on_double_call(self):
        ...
        # Simulate sync + async dual-fire for the same request
        otel._handle_success(kwargs, response_obj=None, start_time=start, end_time=end)
        otel._handle_success(kwargs, response_obj=None, start_time=start, end_time=end)
    ...
    def test_handle_success_dedupe_skip_still_closes_proxy_span(self):
        ...
        # Pre-mark to force the dedupe-skip branch.
        otel._mark_success_span_once(kwargs)
        self.assertTrue(proxy_span.is_recording())
...
class TestOpenTelemetryFailureSpanDedupe(unittest.TestCase):
    """streaming code paths trigger both sync and async failure callbacks for one request,
    causing duplicate ERROR litellm_request spans. _mark_failure_span_once provides a request-local
    idempotency guard symmetric with the success-side fix."""
    ...
    def test_mark_failure_span_once_first_call_returns_true(self):
        otel = OpenTelemetry()
        kwargs = self._build_kwargs()
        self.assertTrue(otel._mark_failure_span_once(kwargs))

    def test_mark_failure_span_once_second_call_returns_false(self):
        otel = OpenTelemetry()
        kwargs = self._build_kwargs()
        self.assertTrue(otel._mark_failure_span_once(kwargs))
        self.assertFalse(
            otel._mark_failure_span_once(kwargs),
            "Repeat call for same handler+kwargs must be deduped",
        )

    def test_mark_failure_span_once_separate_handlers_each_emit(self):
        """Two distinct handler instances must each emit exactly once."""
        otel_a = OpenTelemetry()
        otel_b = OpenTelemetry()
        kwargs = self._build_kwargs()
        self.assertTrue(otel_a._mark_failure_span_once(kwargs))
        self.assertTrue(
            otel_b._mark_failure_span_once(kwargs),
            "Different handler instance must not share the first handler's marker",
        )
        self.assertFalse(otel_a._mark_failure_span_once(kwargs))
        self.assertFalse(otel_b._mark_failure_span_once(kwargs))

    def test_success_and_failure_dedupe_are_independent(self):
        """A success marker must not suppress a failure marker on the same
        handler (and vice-versa). Outcome-specific dedupe keys ensure each
        outcome emits exactly one span per request."""
        otel = OpenTelemetry()
        kwargs = self._build_kwargs()
        self.assertTrue(otel._mark_success_span_once(kwargs))
        self.assertTrue(
            otel._mark_failure_span_once(kwargs),
            "Failure dedupe must be independent of the success dedupe",
        )
        self.assertFalse(otel._mark_success_span_once(kwargs))
        self.assertFalse(otel._mark_failure_span_once(kwargs))

    def test_mark_failure_span_once_handles_missing_metadata(self):
        otel = OpenTelemetry()
        kwargs = {"litellm_params": {}}
        self.assertTrue(otel._mark_failure_span_once(kwargs))
        self.assertFalse(otel._mark_failure_span_once(kwargs))

    def test_mark_failure_span_once_handles_missing_litellm_params(self):
        otel = OpenTelemetry()
        kwargs = {}
        self.assertTrue(otel._mark_failure_span_once(kwargs))
        self.assertFalse(otel._mark_failure_span_once(kwargs))
